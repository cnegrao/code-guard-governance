...
## 0-TER. CANONICAL PROFILE IMMUTABILITY / ENRICHMENT CORRECTION

A further external review of the implemented Technical Profile Persistence V1 (§0-BIS) found one remaining gap: the original `materialize_agent_version_technical_profile` correctly protected against (a) an identical materialization replay and (b) a reused `proposal_id` carrying incompatible content (§ PROPOSAL_IDEMPOTENCY_RULE), but did **not** protect against a **different** `proposal_id` carrying a **different `behaviorFingerprint`** materialized against the **same** canonical AGENT_VERSION — that case would previously have silently overwritten the governed profile's fingerprint via the `ON CONFLICT ... DO UPDATE`. This violates the core invariant this ADR itself depends on: **CANONICAL AGENT_VERSION = ONE technical/behavioral revision** — a different fingerprint describes a different technical revision, which belongs on a different canonical AgentVersion, never an update of this one.

**Fixed, without reopening or amending the accepted architecture decision** (ADR Decision F is *extended* with sub-decisions F.1–F.5, not replaced — see `ADR-GOVIA-TECHNICAL-PROFILE-PERSISTENCE-v1.md`):

1. **behaviorFingerprint immutability**: a mismatch against an already-governed profile now fails closed (`AGENT_VERSION_TECHNICAL_REVISION_MISMATCH`) before any write; the fingerprint columns are structurally never present in the function's `ON CONFLICT ... DO UPDATE SET` clause.
2. **Monotonic optional-field enrichment**: `buildReference`/`runtimeFrameworkReference`/`entrypointReference`/`configurationReference` now follow UNKNOWN→known (enrichment), known→same (compatible), known→UNKNOWN (preserved, never erased), known→different (`AGENT_VERSION_PROFILE_SEMANTIC_CONFLICT`, fails closed — never last-write-wins).
3. **Monotonic provenance union**: field-level assertion/evidence support is now `INSERT ... ON CONFLICT DO NOTHING` (never delete-then-replace), so enrichment can never destroy previously governed support.
4. **Real-change-gated revision**: `revision` now increments only when a field actually transitions from UNKNOWN to known or new support is actually added — never merely because a different (but compatible, uninformative) `proposal_id` was materialized.
5. **`source_proposal_id` = ORIGIN only**: clarified (not changed in shape) to record only the first materializing proposal, never overwritten by later compatible enrichments — full field provenance remains the union in the field-support tables; the full proposal sequence remains the append-only materializations audit trail.

See §8's `TECHNICAL_REVISION_IMMUTABILITY_RULE` / `PROFILE_ENRICHMENT_RULE` / `PROVENANCE_UNION_RULE` / `REVISION_RULE` / `SOURCE_PROPOSAL_ID_SEMANTICS` subsections below for the full detail, and §16 for the new migration test file that statically proves all of the above against the corrected SQL.

Additionally, while writing that migration test file against the established sibling-migration test pattern (which always asserts EXECUTE is revoked from `public`/`anon`/`authenticated` and granted only to `service_role` on every new function), two genuine, independent, pre-existing structural defects were found and fixed in the same migration file:

- **Missing EXECUTE revoke/grant**: `record_agent_version_technical_profile_proposal`/`materialize_agent_version_technical_profile` had no `revoke all on function ... from public, anon, authenticated` / `grant execute ... to service_role` statements at all — every other function in this migration history has them (PostgreSQL grants `EXECUTE` to `PUBLIC` by default on new functions). **Fixed**: added, identical in form to every prior migration's own permissions section.
- **NAMEDATALEN (63-byte) identifier overflow**: several constraint names (e.g. `agent_version_technical_profile_proposal_field_assertions_assertion_fkey`, 72 bytes), one index name, and all seven RLS policy names (e.g. `"Service role has full access to agent_version_technical_profile_materializations"`, 80 bytes) exceeded PostgreSQL's 63-byte identifier limit — PostgreSQL silently truncates rather than erroring, which the sibling migration test files already explicitly guard against. **Fixed**: a redundant `UNIQUE(organisation_id, proposal_id)` constraint duplicating the primary key on `agent_version_technical_profile_proposals` was deleted outright (dead weight, zero behavior change); the two `proposal_field_assertions`/`proposal_field_evidence` tables' constraint/index names adopt an `avtp_proposal_field_*` abbreviation prefix instead of the full table name; all seven RLS policy names adopt an `avtp_*` abbreviation. Table names themselves were never touched — only the identifier strings of constraints, one index, and policies, none of which are referenced by name from any TypeScript code. Verified: zero identifiers exceed 63 bytes anywhere in the corrected file (programmatic scan, §16).
...
The originally-implemented `materialize_agent_version_technical_profile` checked `agent_version_technical_profile_materializations` for an existing row but performed the `ON CONFLICT DO UPDATE` upsert (which unconditionally increments `revision`) **before** consulting that check — meaning a pure replay of an already-applied `(canonical_object_id, proposal_id)` pair would have incorrectly bumped `revision`, implying enrichment that never happened. **Fixed**: the existing-materialization check now runs first and returns immediately (`replay: true`, no writes at all) before the upsert, field-support union, audit-row insert, or outbox-event insert are ever reached.

### TECHNICAL_REVISION_IMMUTABILITY_RULE (new)

**CANONICAL AGENT_VERSION = ONE technical/behavioral revision** (ADR §F.1). For an already-governed canonical profile, `behaviorFingerprint` (`_algorithm`/`_schema_version`/`_value`) is semantically immutable: a materialize call whose proposal carries a fingerprint that differs from the profile already on file is not describing an enrichment of that profile — it is describing a different technical revision, which belongs on a different canonical AgentVersion (a Discovery/reconciliation-stage concern this function has no authority over). `gov_repo.materialize_agent_version_technical_profile` now loads the existing profile (if any) immediately after the idempotency replay check and, before any write, compares its fingerprint against the incoming proposal's; a mismatch raises `AGENT_VERSION_TECHNICAL_REVISION_MISMATCH` (fails closed — no revision bump, no field-support write, no audit row, no outbox event). This is enforced structurally, not only by the runtime check: the fingerprint columns are never present in the function's `ON CONFLICT (organisation_id, canonical_object_id) DO UPDATE SET` clause, so no code path in this function can ever write a fingerprint value once a profile row exists — confirmed by a static test (§16) that the UPDATE branch's SET clause contains no `behavior_fingerprint_*` assignment.

### PROFILE_ENRICHMENT_RULE (new)

For the SAME `behaviorFingerprint`, enrichment of the four optional fields (`buildReference`/`runtimeFrameworkReference`/`entrypointReference`/`configurationReference`) is monotonic (ADR §F.2): `UNKNOWN → known` is enrichment; `known → the SAME known value` is a compatible no-op; `known → UNKNOWN` never erases already-governed knowledge (an existing non-null value always wins over an incoming `NULL`); `known → a DIFFERENT known value` fails closed with `AGENT_VERSION_PROFILE_SEMANTIC_CONFLICT` — last-write-wins is explicitly forbidden. Implemented per-field as `v_final_<field> := coalesce(v_existing_profile.<field>, v_proposal.<field>)`, guarded by an explicit conflict check (`existing is not null AND incoming is not null AND existing IS DISTINCT FROM incoming` → raise) that runs before the `coalesce` and before any write. The conflict error's `detail` identifies only the field name, never the conflicting values themselves.

### PROVENANCE_UNION_RULE (new)

Field-level `assertionIds`/`evidenceIds` support is a monotonic UNION across every compatible proposal ever materialized against a canonical profile (ADR §F.3), never a delete-then-replace. The prior implementation deleted all of a profile's existing field-support rows before re-inserting the current proposal's rows — correct only for a first-ever materialization, but destructive for any subsequent enrichment (it would have silently discarded provenance contributed by an earlier compatible proposal). **Fixed**: the `delete from ... field_assertions`/`delete from ... field_evidence` statements were removed entirely; the insert now targets the canonical field-support tables directly with `on conflict do nothing`, relying on those tables' own primary keys `(organisation_id, canonical_object_id, field_name, assertion_id|evidence_id)` for deduplication. A proposal materialized purely to add new support to an already-governed field now adds to existing membership instead of replacing it.

### REVISION_RULE (new)

`revision` increments only for an actual governed change (ADR §F.4): the first-ever profile for an AgentVersion; a previously-`UNKNOWN` optional field becoming known; or newly added assertion/evidence support rows (detected via `GET DIAGNOSTICS ... = ROW_COUNT` immediately after each field-support insert — a nonzero count means at least one row was genuinely new, not merely a duplicate absorbed by `ON CONFLICT DO NOTHING`). A `v_changed` boolean accumulates all three signals; the canonical-profile upsert's `revision`/`updated_at` assignments are both gated on it (`case when v_changed then ... else <unchanged> end`). A compatible proposal that changes nothing observable is still accepted and produces its own audit row (a genuinely distinct `proposal_id` was successfully processed) and its own outbox event, but `revision` itself does not advance — `revision` counts real enrichment events, not proposal submissions.

### SOURCE_PROPOSAL_ID_SEMANTICS (new)

`agent_version_technical_profiles.source_proposal_id` records only the ORIGIN proposal — the one that first materialized the canonical profile row (ADR §F.5). It is not overwritten by any later compatible enrichment: the column is absent from the `ON CONFLICT ... DO UPDATE SET` clause entirely, exactly like the fingerprint columns. This was already the column's only sane reading (a "last writer" semantic would require an explicit new decision this correction is not making), so nothing about the column's shape or meaning changed — only the previously-missing UPDATE-branch omission needed fixing (the original code also never wrote `source_proposal_id` in the SET clause, so this was already correct; it is called out here explicitly, per the reviewer's request, rather than left implicit). Full field-level provenance is the union recorded in the field-support tables (`PROVENANCE_UNION_RULE` above); the complete sequence of every proposal ever applied to a profile is `agent_version_technical_profile_materializations`, an append-only audit trail keyed by `(organisation_id, canonical_object_id, proposal_id)`.

### MIGRATION_TEST (new)

`apps/dashboard/tests/agent-version-technical-profile-persistence-migration.test.ts` (new, 32 tests) statically asserts, directly against the corrected migration SQL text (never a parallel reimplementation that could diverge from real SQL semantics): both tables carry typed, unrenamed contract columns; no table column anywhere in the migration is typed `jsonb`; every `field_name` CHECK enumerates exactly the 5 frozen support field names; RLS is enabled and only a `service_role` policy exists on every new table; the canonical-target-kind and active-source-mapping governance gates are present; the materializations replay check runs textually before the first write; the fingerprint-mismatch guard raises `AGENT_VERSION_TECHNICAL_REVISION_MISMATCH` and the fingerprint columns are absent from the UPDATE SET clause; each of the 4 optional fields is guarded by a non-null-vs-non-null distinct check before any write (`AGENT_VERSION_PROFILE_SEMANTIC_CONFLICT`); each field's final value is `coalesce(existing, incoming)`; field-support inserts use `ON CONFLICT DO NOTHING` with no `DELETE` statement against either field-support table; `revision`'s SET clause is `v_changed`-gated and `v_changed` is derived from real field/support deltas; the replay early-return textually precedes the outbox insert; the outbox CHECK constraint contains exactly 6 event-type literals (the prior 5 plus the 1 new one); `source_proposal_id` is absent from the UPDATE SET clause; every constraint/index/policy/table/function identifier is ≤63 bytes; EXECUTE is revoked from `public`/`anon`/`authenticated` and granted only to `service_role` on both new functions; and no prior migration file's own domain was touched. This is the domain-level proof required by the review (items A–G): because the enrichment/immutability logic exists exclusively inside this one SQL function (no parallel TypeScript reimplementation exists to test independently — Discovery Intake only records proposals, never materializes), these structural assertions against the actual SQL are the faithful, non-duplicative way to prove the required behavior without constructing a fake model that could contradict real SQL semantics.
...
One new, purely additive migration: `supabase/migrations/20260908120000_agent_version_technical_profile_persistence_v1.sql` (887 lines after the §0-TER corrections). A `do $preflight$` block fails closed (raises, does not silently skip) if `gov_repo` or any of its prerequisite tables (`canonical_objects`, `canonical_object_source_mappings`, `discovery_candidates`, `discovery_evidence`, `source_assertions`, `outbox_events`) are missing, and again if any target table already exists (naming-collision guard). No historical migration file was edited; `canonical_objects`, `canonical_relationships`, `materialization_operations`, `materialization_locks`, `discovery_findings`, `discovery_candidates` are untouched by this migration. EXECUTE on both new functions is revoked from `public`/`anon`/`authenticated` and granted only to `service_role` (added in §0-TER — this was missing from the original migration despite being present in every prior sibling migration). Every constraint/index/policy identifier is ≤63 bytes (also corrected in §0-TER: a redundant duplicate-of-the-primary-key UNIQUE constraint was deleted, two tables' constraint/index names adopt an `avtp_proposal_field_*` abbreviation, and all seven RLS policy names adopt an `avtp_*` abbreviation — table names themselves are unchanged). Not applied to any live/production Supabase instance — no live database access was used or is authorized by this pass; internal consistency was verified by full re-read of the corrected file, a programmatic byte-length scan of every identifier, plus the new migration test file and the TypeScript-level port/adapter tests (§16), not by execution against a running database.
...
### 13.1 — Files changed (§0-TER: canonical profile immutability/enrichment correction, on top of `916c8bb`)

Modified:
- `supabase/migrations/20260908120000_agent_version_technical_profile_persistence_v1.sql` — `materialize_agent_version_technical_profile` gains the behaviorFingerprint-immutability check, monotonic optional-field enrichment, provenance-union field-support inserts, and `v_changed`-gated revision (see §8's new subsections); the redundant `UNIQUE(organisation_id, proposal_id)` constraint on `agent_version_technical_profile_proposals` was deleted; `avtp_proposal_field_*`/`avtp_*` identifier abbreviations were introduced for constraints/index/policies exceeding 63 bytes; EXECUTE revoke/grant statements were added for both functions
- `docs/architecture/ADR-GOVIA-TECHNICAL-PROFILE-PERSISTENCE-v1.md` — Decision F extended with sub-decisions F.1–F.5 (technical-revision immutability, monotonic enrichment, provenance union, revision semantics, source_proposal_id semantics); no other section rewritten
- `docs/codex/evidence/2026-09-08-agent-technical-profile-l4-round1-validation.md` — this document (§0-TER, new §8 subsections, §13.1, §16 additions)

Added:
- `apps/dashboard/tests/agent-version-technical-profile-persistence-migration.test.ts` — 32 static structural tests against the corrected migration SQL (§ MIGRATION_TEST)

No file under `docs/architecture/GOVIA-L0L16-CIA-v1.0.md`, `.claude/**`, `codex-recovery-6101-6240.txt`, `packages/scanner/**`, `packages/governance-review/**` (other than as already listed), or `packages/scanner/test/discovery-validation-lab/**` was touched by this correction. No dependency, lockfile change. No amendment to `deef88a`/`2769f1e`/`88b0c60`/`916c8bb`.
...
| Migration SQL (recovery-session pass) | full re-read of the corrected 725-line file end-to-end | internally consistent; no live-database execution (none authorized) |
| `git diff --check` | repo root | clean |
| **§0-TER — Migration SQL (this correction pass)** | full re-read of the corrected 887-line file end-to-end + programmatic identifier byte-length scan | internally consistent; zero identifiers exceed 63 bytes; no live-database execution (none authorized) |
| **§0-TER — new migration test file** | `node --test tests/agent-version-technical-profile-persistence-migration.test.ts` (apps/dashboard) | **32/32 passing** |
| **§0-TER — Dashboard workspace typecheck (re-confirmed)** | `npx tsc --noEmit` (apps/dashboard) | clean |
| **§0-TER — `discovery-intake-service.test.ts` (re-confirmed, unaffected)** | `node --test tests/discovery-intake-service.test.ts` (apps/dashboard) | **28/28 passing** |
| **§0-TER — `git diff --check`** | repo root | clean |

`canonical-contracts` and all historical migrations were not touched, so their own suites were not re-run. Neither this correction pass nor the prior recovery-session pass touched any scanner or governance-review TypeScript file, so those suites were not re-run a third/second time — only the migration itself, its new test file, and the dashboard suites that could plausibly be affected were re-verified, run once per the task's own instruction not to re-run merely because it is a new session.
...
- **Canonical profile immutability/enrichment logic (§0-TER) is proved only statically** — by direct assertion against the migration's own SQL text (§ MIGRATION_TEST), not by executing it against a running Postgres instance, since no live Supabase/production access is authorized for this milestone. The static assertions examine the real SQL directly (never a parallel TypeScript reimplementation), which is the reason this is considered sufficient rather than merely convenient — but it is not the same guarantee an executed integration test against a real database would provide.
